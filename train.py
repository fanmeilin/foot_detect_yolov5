...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if isinstance(hyp, str):
        with open(hyp) as f:
            hyp = yaml.safe_load(f)  # load hyps dict 从yaml文件中读取超参数
    ...
    if pretrained:
        ...
        ckpt = torch.load(weights, map_location=device)  # load checkpoint 加载checkpoint
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default='data/foot_detect.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--workers', type=int, default=0, help='maximum number of dataloader workers')  #改为0
